...
const TOOL_METADATA: Record<string, { label: string, icon: any, color: string, bgColor: string }> = {
  ...
  symptom_decoder: { label: 'Safety Check', icon: Stethoscope, color: 'text-rose-500', bgColor: 'bg-rose-100' },
};
  ...
  const renderTool = () => {
    switch (activeCategory) {
      ...
      case 'symptom_decoder':
        const diff2 = new Date().getTime() - new Date(profile.lmpDate).getTime();
        const weeks2 = Math.floor(diff2 / (1000 * 60 * 60 * 24 * 7));
        const currentTrimester2 = weeks2 < 13 ? Trimester.FIRST : weeks2 < 27 ? Trimester.SECOND : Trimester.THIRD;
        return <SymptomDecoder trimester={currentTrimester2} />;
      case 'reports':
        return <ReportCenter />;
      case 'export':
        return <ExportReport 
