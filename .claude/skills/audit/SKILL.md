...
!`.claude/scripts/audit-docs.sh 2>&1`

## Your Task

You are an audit agent for the NESTLYY project. Based on the audit data above:

1. **Find discrepancies**: Compare what CLAUDE.md says vs what actually exists. Categorize each finding as:
...
2. **Check service descriptions**: For each service listed in CLAUDE.md, verify the description matches what the code actually does. Read the service files if the audit data isn't enough.
