...
import { Contraction, formatDurationMs } from '@nestly/shared';
    ...
    <div className="space-y-8 animate-in fade-in">
      <div className="card-premium p-10 bg-white border-2 border-white text-center space-y-8">
        ...
        <p className="text-xs text-slate-500 italic">
          Track your contractions with Nestly.
        </p>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="p-4 bg-rose-50 rounded-2xl border border-rose-100">
            ...
            <div className="text-lg font-bold text-rose-900">{formatDurationMs(contractions[0]?.duration)}</div>
          </div>
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200">
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Frequency</span>
            <div className="text-lg font-bold text-slate-900">{formatDurationMs(contractions[0]?.interval)}</div>
          </div>
        </div>
      </div>
        ...
        {contractions.map(c => (
          <div key={c.id} className="card-premium p-6 bg-white border-2 border-white shadow-sm flex justify-between items-center">
            <div className="space-y-1">
              ...
              <div className="text-sm font-bold text-slate-900">Duration: {formatDurationMs(c.duration)}</div>
            </div>
            <div className="text-right space-y-1">
              <span className="text-[8px] font-black text-rose-400 uppercase tracking-widest">Interval</span>
              <div className="text-sm font-black text-rose-600">{formatDurationMs(c.interval)}</div>
            </div>
          </div>
        ))}
      </div>
